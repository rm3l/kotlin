...
object ExpectedActualDeclarationChecker : DeclarationChecker {
        ...
        return when (expected) {
            is CallableMemberDescriptor -> {
                expected.findNamesakesFromModule(platformModule).filter { actual ->
                    expected != actual && !actual.isHeader &&
    ...
    private fun checkActualDeclarationHasExpected(
        ...
        val hasExpectedModifier = descriptor.isImpl &&
                                  (reportOn.hasModifier(KtTokens.IMPL_KEYWORD) || reportOn.hasModifier(KtTokens.ACTUAL_KEYWORD))
        ...
        return when (actual) {
            is CallableMemberDescriptor -> {
                ...
                candidates.filter { declaration ->
                    actual != declaration && declaration.isHeader
                }.groupBy { declaration ->
