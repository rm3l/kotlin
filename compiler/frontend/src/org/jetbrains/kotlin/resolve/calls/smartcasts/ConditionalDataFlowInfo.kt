...
class ConditionalDataFlowInfo(val thenInfo: DataFlowInfo, val elseInfo: DataFlowInfo = thenInfo) {
    fun and(other: ConditionalDataFlowInfo): ConditionalDataFlowInfo {
        ...
        return ConditionalDataFlowInfo(
                this.thenInfo.and(other.thenInfo),
                this.elseInfo.and(other.elseInfo)

        )
    }

    fun or(other: ConditionalDataFlowInfo): ConditionalDataFlowInfo {
        if (this == EMPTY) return other
        if (other == EMPTY) return this

        return ConditionalDataFlowInfo(
                this.thenInfo.or(other.thenInfo),
                this.elseInfo.or(other.elseInfo)
